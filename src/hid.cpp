...
int hid_init()
{
    ...
    for (DWORD i = 0;
         SetupDiEnumDeviceInterfaces(set, nullptr, &hidGuid, i, &ifd);
         ++i)
    {
        DWORD need = 0;
        ...
        if (hDev == INVALID_HANDLE_VALUE) { closeCurrent(); continue; }

        if (!HidD_GetPreparsedData(hDev, &prep)) { closeCurrent(); continue; }

        HIDP_CAPS caps{};
        if (HidP_GetCaps(prep, &caps) != HIDP_STATUS_SUCCESS) { closeCurrent(); continue; }
        ...
        /* Input */
        if (HidP_GetValueCaps(HidP_Input, v, &len, prep) != HIDP_STATUS_SUCCESS ||
            len == 0 || v[0].ReportCount != 1)
        { closeCurrent(); continue; }
        ...
        /* Feature */
        len = 4;
        if (HidP_GetValueCaps(HidP_Feature, v, &len, prep) != HIDP_STATUS_SUCCESS ||
            len == 0 || v[0].ReportCount != 1)
        { closeCurrent(); continue; }
...
{
    if (hDev == INVALID_HANDLE_VALUE) return -1;
    uint8_t buf[100]{}; buf[0] = inCaps.id;
    if (!HidD_GetInputReport(hDev, buf, sizeof(buf))) return -2;
    NTSTATUS s = HidP_GetUsageValue(HidP_Input,
                                    inCaps.page, 0, inCaps.usage,
                                    val, prep,
                                    reinterpret_cast<PCHAR>(buf), inCaps.len);
    return (s == HIDP_STATUS_SUCCESS) ? 0 : -3;
}
int hid_setBrightness(ULONG v)
{
    if (hDev == INVALID_HANDLE_VALUE) return -1;
    uint8_t buf[100]{}; buf[0] = featCaps.id;
    if (!HidD_GetFeature(hDev, buf, sizeof(buf))) return -2;
    NTSTATUS s = HidP_SetUsageValue(HidP_Feature,
                                    featCaps.page, 0, featCaps.usage,
                                    v, prep,
                                    reinterpret_cast<PCHAR>(buf), featCaps.len);
    if (s != HIDP_STATUS_SUCCESS) return -3;
    return HidD_SetFeature(hDev, buf, featCaps.len) ? 0 : -4;
}
int hid_getBrightnessRange(ULONG* mn, ULONG* mx)
{
    if (!prep || hDev == INVALID_HANDLE_VALUE) return -1;
    HIDP_VALUE_CAPS v{}; USHORT len = 1;
    if (HidP_GetValueCaps(HidP_Feature, &v, &len, prep) != HIDP_STATUS_SUCCESS || len == 0)
        return -2;
    *mn = v.LogicalMin; *mx = v.LogicalMax; return 0;
}
void hid_deinit() { closeCurrent(); }
